...
<?php $__env->stopSection(); ?> <?php $__env->startSection('sidebar'); ?>
<li class="treeview active">
  ...
  <ul class="treeview-menu">
    <li>
      <a href="/All">
        <i class="fa fa-circle-o active"></i> همه شکایات</a>
    </li>
    <li>
      <a href="#">
        <i class="fa fa-circle-o"></i> دسته ها
        <i class="fa fa-angle-left pull-right"></i>
      </a>
      <ul class="treeview-menu">
        <li>
          <a href="/Cat/1">
            <i class="fa fa-circle-o"></i> پیگیری</a>
        </li>
        <li>
          <li>
            <a href="/Cat/2">
              <i class="fa fa-circle-o"></i> حقوقی</a>
          </li>
      </ul>
    </li>
  </ul>
        ...
        <div class="box box-primary">
          <div class="box-body">
            ...
            <form method="get" id="trackForm" action="#" style="display:inline-block;float:left;display:none;">
              <button type="submit" class="btn btn-default btn-sm newTrack" style="background-color: #239D60;font-size: 13px;height: 32px;margin-top: 7px;color: white;"
                type="button">
            ...
            <button class="btn btn-default btn-sm newComplaint" style="     background-color: #239D60;font-size: 13px;height: 32px;margin-top: 5px;color: white;"
              type="button">
    ...
    <div class="remodal" data-remodal-id="modal" style="max-width: 100%;width: 95%;">
      ...
      <h4 style="margin: 0 auto;margin-bottom: 15px">: تاریخچه شکایات بارنامه
        <span id="selConsignment" style="color:#3c8dbc;font-weight: bold;"></span>
      </h4>
      <div id="modalTable">
      </div>
      <br>
    ...
    <div class="remodal" data-remodal-id="createModal" style="max-width: 100%;width: 95%;">
      <button data-remodal-action="close" class="remodal-close"></button>
      <h4 style="margin: 0 auto;margin-bottom: 15px;text-align:center;color:#2196F3">ثبت شکایت جدید
        <span id="selConsignment" style="color:#3c8dbc;font-weight: bold;"></span>
      </h4>
      ...
      <form id="newComplaintForm" action="newComplaint" method="get" style="text-align:right;">
        <div class="form-group">
          ...
          <img style="height:40px;width:40px;display:none;" id="laoderImage" src="<?php echo e(asset('img/Spinner.gif')); ?>" />
      ...
      $(document).ready(function () {
          ...
          'ajax': {
            ...
            "dataSrc": function (json) {
              ...
              for (var i = 0; i < json.data.length; i++) {
                ...
                var selectedCat = json.data[i].fld_Cat;
                var catOptions = "";
                for (var ik = 0; ik < json.cats.length; ik++) {
                  if (json.cats[ik].fld_Id == selectedCat) {
                    catOptions += " <option value=" + '"' + json.cats[ik].fld_Id + '"' + "selected" + ">" +
                      json.cats[ik].fld_Cat_Name + "</option> "
                  } else {
                    catOptions += " <option value=" + '"' + json.cats[ik].fld_Id + '"' + ">" + json.cats[ik].fld_Cat_Name +
                      "</option> "
                  }
                }
                ...
                return_data.push({
                  ...
                  'fld_Level': "<select class=" + '"' + "form-control level" + '"' + "name=" + '"' + json
                    .data[i].fld_Id + '"' + ">" + "<option value=" + '"' + "1" + '"' + selected1 +
                    ">ثبت شده است</option>" + "<option value=" + '"' + "2" + '"' + selected2 +
                    ">در حال انجام</option>" + "<option value=" + '"' + "3" + '"' + selected3 +
                    ">به اتمام رسیده است</option>" + "</select>",
                  'fld_Cat': "<select class=" + '"' + "form-control  cat" + '"' + "name=" + '"' + json.data[
                    i].fld_Id + '"' + ">" + catOptions + "</select>",
                  'DT_Row_Index': json.data[i].DT_Row_Index,
                  'fld_Complaints_Subjects': json.data[i].fld_Complaints_Subjects,
                  'options': "<button value =" + '"' + json.data[i].fld_Consignment + '"' + "class =" +
                    '"' + "btn btn-default btn-sm history" + '"' + "><span class=" + '"' +
                    "glyphicon glyphicon-repeat" + '"' + "></span> تاریخچه" + "<span class=" + '"' +
                    "countOfRecords" + '"' + ">" + json.data[i].count + "</span> </button>" +
                    "<button value =" + '"' + json.data[i].fld_Consignment + '"' + "class =" + '"' +
                    "btn btn-default btn-sm newComplaintSpc" + '"' + "><span class=" + '"' +
                    "glyphicon glyphicon-plus" + '"' + "></span> شکایت جدید</button>",
        ...
        $(function () {
          ...
          $('#newComplaintForm').on('submit', function (e) {
            ...
            if ($('#Consignment').val().substr(0, 7) != 5410000 || $('#Consignment').val().substr(14, 3) !=
              101) {
              ...
              if (CheckConExsist) {
                $.ajax({
                  ...
                  success: function () {
                    ...
                    toastr.success('شکایت با موفقیت ثبت شد', {
                      timeOut: 7000
                    });
                    $('p').trunk8({
          ...
          $(document).on('click', '.deleteComplainBtn', function (e) {
            ...
            $.ajax({
              ...
              success: function (data) {
                ...
                toastr.success('شکایت با موفقیت حذف شد ', {
                  timeOut: 7000
                });
                $globHistory = "";
              }
          ...
          $(document).on('change', '.level', function (e) {
            ...
            $.ajax({
              ...
              success: function (data) {
                toastr.success('بارنامه ' + ": " + $con, 'مرحله شکایت تغیر پیدا کرد', {
                  timeOut: 7000
                });
                $('p').trunk8({
                  fill: '&hellip; <a id="read-more" href="#">بیشتر</a>'
          ...
          $(document).on('change', '.cat', function (e) {
            ...
            $.ajax({
              method: "GET",
              url: "<?php echo e(URL::to('/ChangeCat')); ?>",
              data: {
                id: $id,
                cat: $cat
              },
              success: function (data) {
                toastr.success('بارنامه ' + ": " + $con, 'دسته شکایت تغیر پیدا کرد', {
                  timeOut: 7000
                });
                $('p').trunk8({
                  fill: '&hellip; <a id="read-more" href="#">بیشتر</a>'
          ...
          $('#newTrack').on('click', function (e) {
            ...
            $("#historyContainer").attr("src", "http://portal.parschapar.local/following.php?tracking=" +
              consignment_no);
